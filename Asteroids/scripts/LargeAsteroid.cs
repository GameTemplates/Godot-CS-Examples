...
public class LargeAsteroid : Node2D
{
	
	private AnimatedSprite sprite;
    private PackedScene mAsteroid;
    ...
    public override void _Ready()
    {
		...
		//get reference to the scene containing medium asteroid
		mAsteroid = (PackedScene)ResourceLoader.Load("res://objects/MediumAsteroid.tscn");
	...
	private void _onCollisionMasksAreaEntered(Node node)
	{
		...
		if(node.Name == "bullet_collision_mask")
		{
			...
			//create 2 medium asteroids in position of this one	
			for(var i = 1; i <= 2; i++)
			{
				var mAsteroidInstance = (Node2D)mAsteroid.Instance();
				mAsteroidInstance.SetPosition(new Vector2(this.Position.x, this.Position.y));
				//GetParent().AddChild(mAsteroidInstance); //old code from 3.0.6, drop error in 3.1 because trying to execute immediately
				GetParent().CallDeferred("add_child", mAsteroidInstance); //in 3.1 defer execution for LATER to add asteroid instance to parent
			}
			...
			particles.Amount = 10; //we emit 10 particles
